...
```s
docker run --rm --network host -e QUARKUS_PROFILE=dev -p 8080:8080 quay.io/puzzle/quarkus-techlab-data-producer:kafka
```
...
```s
docker run --rm --network host -e QUARKUS_PROFILE=dev -p 8080:8080 quay.io/puzzle/quarkus-techlab-data-consumer:kafka
```
